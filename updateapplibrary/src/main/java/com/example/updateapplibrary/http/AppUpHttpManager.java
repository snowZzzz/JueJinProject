package com.example.updateapplibrary.http;

import android.os.AsyncTask;
import com.example.updateapplibrary.util.SSLSocketFactoryUtils;
import java.io.File;
 ...
 */
public class AppUpHttpManager implements IAPPUPHttpManager {

    private static final int DEFAULT_TIME_OUT = 20000;

    private int mTimeout = DEFAULT_TIME_OUT;

    private static AppUpHttpManager INSTANCE;

    public static AppUpHttpManager getInstance(){
        if(INSTANCE == null){
            synchronized (AppUpHttpManager.class){
                INSTANCE = new AppUpHttpManager();
            }
        }
    ...
    private AppUpHttpManager(){
        this(DEFAULT_TIME_OUT);
    }

    public AppUpHttpManager(int timeout){
